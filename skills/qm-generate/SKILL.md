...
## 成员权限控制（memberPermissions）

QM 通过 `memberPermissions[]` 控制 synthetic member-QM 的维度成员查询权限，覆盖 TM 的 `dimensions[].memberPermission` 默认配置。

### 与 accesses 的区别

| 机制 | 作用对象 | 使用场景 |
|------|---------|---------|
| `accesses` | 事实数据查询（QM 主链） | 控制业务数据行级访问 |
| `memberPermissions` | 维度成员查询（synthetic member-QM） | 控制维度下拉选项的可见范围 |
    ...
    memberPermissions: [
        {
            ...
            queryBuilder: (context) => {
                ...
                query.and(member.tenantId, context.security.tenantId);
            }
        }
    ],
...
- 如果 TM 维度已有 `memberPermission`，QM 需要按业务场景收窄 → 生成 `memberPermissions[]`
- 如果维度成员不需要权限控制 → 不生成 `memberPermissions`
- `dimension` 字段使用 TM 中的维度 `name`（如 `'product'`、`'store'`）
