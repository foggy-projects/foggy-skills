...

{
    ...
    dimensionDataSql: (context) => {
        ...
        if (!token || !token.tenantId) {
            throw new Error('维度成员查询必须携带 tenantId');
        }
        return `SELECT org_id AS id, org_name AS caption FROM x3_basic.org WHERE tenant_id = ${token.tenantId} ORDER BY org_name`;
    }
}
```

**`dimensionDataSql` 说明**：
- 作用于维度下拉选项查询（`/jdbc-model/dimension/v2/{model}/{dimension}`），过滤可选成员
- 不加 → 维度下拉展示全表数据，租户 A 能看到租户 B 的机构/门店/员工等
- 返回 SQL 必须包含 `id` 和 `caption` 两列（别名）
- 租户端模型中，无 token 必须 throw Error，不能降级

**决策规则**：
- 维度字段需要在前端表格中展示名称（如机构名、客户名）→ 用完整维度
- 完整维度的维度表含多租户数据 → **必须加 `dimensionDataSql` 做租户隔离**
