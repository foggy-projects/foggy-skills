...
如果验收对象属于某个版本或迭代，应先确认该版本是否已有“版本目标 / 迭代目标 / 成功标准”文档，并以其作为上游验收基线；不能把局部功能存在直接等同于版本目标达成。
...
当验收对象涉及 UI 交互功能时（页面、表单、列表、弹窗、导航、按钮交互、数据展示等），验收必须额外核对：

### 必须存在的证据

- **体验检查清单**：覆盖页面可达性、核心交互流程、表单验证、异常状态、权限可见性、数据一致性
- **playwright 自动化测试通过记录**：UI 功能必须有 playwright 测试覆盖，手工走查仅作补充
- playwright 测试必须可独立运行、可重复执行

### 验收检查项

- [ ] 体验检查清单每个维度是否都有对应 playwright test case
- [ ] playwright 测试是否全部通过
- [ ] 如有 playwright 未覆盖的体验维度，是否有手工走查截图或记录作为补充
- [ ] 如 playwright 环境不可用，是否在 progress 中标注原因（此时不得标记体验验证完成）

### 判定规则

- UI 功能缺少 playwright 测试通过记录 → 体验验证不完整 → 至少标为 `blocked` 或 `accepted-with-risks`
- 有 playwright 测试但未运行 → 等同于测试缺失
- 纯后端 / 纯 API / 无 UI 功能 → 体验验证标 `N/A`，无需 playwright
