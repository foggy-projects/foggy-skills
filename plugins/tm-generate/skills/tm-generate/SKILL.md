...
- **字典检测**（重要）：如果能获取到 Java Entity 源码，检查 `@DictRef(Xxx.class)` 或 `@DictRefs` 注解，有则在 TM 属性上加 `dictRef: 'Xxx'`。不加会导致 frontend-meta 把字典列当普通数字列，前端筛选器变成"最小/最大"而非枚举下拉
...
- [ ] 已识别并配置维度
- [ ] 使用维度构建器（如有可复用维度）
- [ ] **字典字段必须添加 dictRef**（检查 Entity 上 `@DictRef` 注解，有则 TM 加 `dictRef: 'Xxx'`；缺失会导致前端把字典列当数字列）
