...
- agent 编码完成后必须先做一次实现自检，再决定是否进入正式质量检查
...
- **实现自检要求（默认强制）**：子 agent 完成编码后，必须先做一次轻量实现自检，至少确认：
  1. requirement / bug scope 是否收口
  2. 是否有明显漏改、误改、临时代码、重复实现
  3. 测试结果、文档回写、风险项是否已记录
  4. 自检结论必须写入 progress，不允许只报“已完成”
- **正式质量检查触发条件**：满足以下任一条件时，编码完成后除自检外还必须执行正式 `foggy-implementation-quality-gate`：
  1. 完成某个阶段、模块或里程碑交付
  2. 涉及跨模块、跨 repo、公共能力或共享契约
  3. 改动面较大，或复杂度已明显上升
  4. BUG 修复存在回归风险，或需要决定是否进一步抽象/收敛
...
- Implementation Self-Check（轻量自检清单与结论）
- 计划外变更（执行中新增/调整的内容）
...
6. 为 root 生成总控执行文档：
   ...
   - 总边界
   - 总实施顺序
   - 执行完成后的 self-check / review / audit / acceptance 流程
...
10. 最后检查：
   ...
   - **是否存在循环依赖风险**
   - 是否明确了测试执行通过、progress 回写、实现自检、最终报告、后置评审 skill
   - 是否和已有文档重复或冲突
...
- 如启用 execution-prompt：包含"编码完成后先做实现自检，再判断是否升级正式质量检查"的指引
